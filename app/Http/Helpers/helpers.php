...
function basics()
{
    $basics = \App\BasicCategory::all()->mapWithKeys(function ($q) {
        return [$q['id'] => $q['name']];
    });
    return $basics;
}
...
function categories($basic =null)
{

    if ($basic != null) {

        $categories = \App\BasicCategory::find($basic)->categories;
    }else
...
function classes($category= null)
{
    if ($category != null) {

        $classes = \App\Category::find($category)->classes->mapWithKeys(function ($item) {
            return [$item['id'] => $item['name']];
        });
    }
    else {
        $classes = \App\Classe::all()->mapWithKeys(function ($item) {
            return ['اختر القسم الفرعى اولا'];
        });
    }
...
function clients()
{
    $cities = App\Client::all()->mapWithKeys(function ($item) {
        return [$item['id'] => $item['name']];
    });
    return $cities;
}
